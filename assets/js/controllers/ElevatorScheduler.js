...
class ElevatorScheduler {
    ...
    scheduleElevator(floor, direction) {
        this.logger.debug(`Scheduling elevator for floor ${floor}, direction ${direction}`);

        const existingCall = this.pendingCalls.find(call =>
            call.floor === floor && call.direction === direction);

        if (existingCall) {
            this.logger.debug(`Call for floor ${floor}, direction ${direction} already pending`);
            return;
        }
        ...
        if (elevator.getCurrentFloor() === floor && !elevator.areDoorsOpen()) {
            ...
            this.logger.debug(`Elevator ${elevator.id} already at floor ${floor}, opening doors`);
            return;
        }
        ...
        this.logger.debug(`Scheduled elevator ${elevator.id} for floor ${floor}, direction ${direction}`);
    }
    ...
    _handleElevatorFloorChange(elevator, newFloor) {
        ...
        if (callsForFloor.length > 0) {
            ...
            this.logger.debug(`Removed calls for floor ${newFloor} from pending calls`);
        }
    }
    ...
    _checkPendingCalls() {
        ...
        this.logger.debug(`Checking ${this.pendingCalls.length} pending calls`);
        ...
        this.pendingCalls.forEach(call => {
            ...
            if (elevator && elevator.isIdle()) {
                ...
                this.logger.debug(`Assigned pending call: floor ${call.floor}, direction ${call.direction} to elevator ${elevator.id}`);
            }
        });
    ...
    reset() {
        this.pendingCalls = [];
        this.logger.debug('Elevator scheduler reset');
    }
}