...
class Building {
    ...
    constructor(config) {
        ...
        this.logger.debug('Building initialized with ' + this.floorCount + ' floors and ' + this.elevatorCount + ' elevators');
    }
    ...
    findClosestElevator(floorNumber, direction) {
        const availableElevators = Object.values(this.elevators).filter(elevator =>
            !elevator.isInEmergency && !elevator.isInMaintenance && !elevator.isOverloaded
        );

        if (availableElevators.length === 0) {
            this.logger.warn('No available elevators');
            return null;
        }
    ...
    reset() {
        ...
        this.logger.info('Building reset to initial state');
